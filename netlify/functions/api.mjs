...
async function ensureSchema() {
  ...
  await db.pool.query("CREATE TABLE IF NOT EXISTS launch_incidents (\n  id TEXT PRIMARY KEY, area TEXT DEFAULT 'Inversores', severity TEXT DEFAULT 'media', title TEXT NOT NULL, detail TEXT, entity_type TEXT, entity_id TEXT, status TEXT DEFAULT 'Abierta', reporter_id INT, assigned_user_id INT, resolved_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())");
  ...
  await db.pool.query("CREATE INDEX IF NOT EXISTS ix_launch_incidents_status ON launch_incidents(status,severity,created_at DESC)");
...
function emailWrap(titulo, cuerpo, cta, ctaUrl){
  if (ctaUrl && String(ctaUrl).charAt(0) === "/") ctaUrl = "https://bravaae.com" + ctaUrl;
  return '<div style="margin:0;background:#F3F5F4;padding:32px 14px;font-family:Arial,Helvetica,sans-serif;color:#0E1116">'
    +'<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #E4E8E6;border-radius:18px;overflow:hidden;box-shadow:0 20px 50px rgba(14,17,22,.08)">'
    +'<tr><td style="height:5px;background:#127A56;font-size:0;line-height:0">&nbsp;</td></tr>'
    +'<tr><td style="padding:30px 34px 20px"><img src="https://bravaae.com/brand/brava-investment-color.png" width="154" alt="BRAVA Investment" style="display:block;width:154px;max-width:100%;height:auto;border:0"><div style="margin-top:13px;color:#127A56;font-size:10px;font-weight:700;letter-spacing:.16em;text-transform:uppercase">Private Investor Access</div></td></tr>'
    +'<tr><td style="padding:4px 34px 34px"><h1 style="font-size:23px;line-height:1.25;color:#0E1116;margin:0 0 14px;letter-spacing:-.02em">' + titulo + '</h1><div style="font-size:15px;color:#3F474D;line-height:1.68;margin:0 0 22px">' + (cuerpo || "") + '</div>'
    +(cta && ctaUrl ? '<a href="' + ctaUrl + '" style="display:inline-block;background:#0E1116;color:#fff;text-decoration:none;padding:13px 23px;border-radius:10px;font-weight:700;font-size:14px;box-shadow:0 5px 0 #050608">' + cta + '</a>' : '')
    +'<div style="height:1px;background:#E8EBE9;margin:31px 0 18px"></div><p style="font-size:11.5px;line-height:1.55;color:#8A9297;margin:0">Comunicación privada y confidencial<br>BRAVA Global Holding Limited · RAK ICC · Reg. ICC-2025-0508 · Dubai, UAE<br><a href="https://bravaae.com" style="color:#127A56;text-decoration:none">bravaae.com</a></p></td></tr></table></div>';
}
/* Utilidades compartidas por las funciones programadas. No exportan secretos:
   los tokens continúan cifrados y solo se descifran durante la ejecución. */
export { sendEmail, emailWrap, mailAccessToken, mailProviderFor, mailAudit };
...
export default async (req) => {
  ...
  try {
    ...
    if (seg[0] === "commercial" && seg[1] === "pipeline" && seg[2] && seg[3] === "draft" && method === "POST") {
      if(!isInternal)return json({error:"Solo equipo interno"},403);const [l]=await db.sql`SELECT * FROM leads WHERE id=${seg[2]}`;if(!l)return json({error:"Lead no encontrado"},404);const b=await req.json().catch(()=>({})),channel=b.channel==="whatsapp"?"whatsapp":"email",first=String(l.nombre||"Contacto").trim().split(/\s+/)[0],division=(l.marca==="realestate"?"Brava Real Estate":(l.marca==="garentto"?"Brava Rent":"BRAVA Investment"));
      let subject="Seguimiento de tu solicitud · "+division,body="Hola "+first+",\n\nGracias por contactar con "+division+". Nos gustaría revisar contigo "+String(l.situacion||l.mensaje||"tu solicitud").replace(/\s+/g," ").slice(0,180)+". ¿Qué momento te viene bien para una breve llamada?\n\nUn saludo,\nEquipo BRAVA";
    ...
    if (path === "investor-operations" && method === "GET" && isInternal) {
      const investments=await db.sql`SELECT id,inversor,email,portal_user_id,proyecto,unidad,estado,fecha_inicio,fecha_fin,plazo_meses FROM inversiones WHERE estado NOT IN ('Cancelada','Cerrada') ORDER BY inversor,fecha_inicio DESC`,items=[];
      for(const inv of investments){const d=await ensureInvestorChecklist(inv.id),pending=d.requirements.filter(r=>r.required&&r.status!=="Validado"),review=d.requirements.filter(r=>r.status==="Pendiente de revisión"),expired=d.requirements.filter(r=>r.status==="Caducado"),appts=inv.portal_user_id?await db.sql`SELECT id,requested_at,status FROM investor_appointments WHERE user_id=${inv.portal_user_id} AND status IN ('Solicitada','Confirmada') ORDER BY requested_at LIMIT 5`:[],milestones=investmentMilestones(invRow(inv)).map(h=>({title:h.titulo,date:h.fecha,days:daysUntil(h.fecha)})),next=milestones.filter(h=>h.days!=null&&h.days>=0).sort((a,b)=>a.days-b.days)[0]||null,priority=expired.length?"critical":(review.length||appts.some(a=>a.status==="Solicitada")||(next&&next.days<=30)||!inv.email||!inv.portal_user_id?"attention":(pending.length?"pending":"ok"));items.push({investmentId:inv.id,investor:inv.inversor,project:inv.proyecto||"Inversión",unit:inv.unidad||"",accessReady:!!(inv.email&&inv.portal_user_id),pendingDocuments:pending.length,inReview:review.length,expiredDocuments:expired.length,appointments:appts.map(a=>({id:a.id,requestedAt:a.requested_at,status:a.status})),nextMilestone:next,priority});}
      const rank={critical:0,attention:1,pending:2,ok:3};items.sort((a,b)=>rank[a.priority]-rank[b.priority]);
      return json({summary:{investors:new Set(items.map(x=>x.investor)).size,contracts:items.length,documentIssues:items.filter(x=>x.pendingDocuments>0).length,reviews:items.reduce((n,x)=>n+x.inReview,0),appointments:items.reduce((n,x)=>n+x.appointments.filter(a=>a.status==="Solicitada").length,0),decisions:items.filter(x=>x.nextMilestone&&x.nextMilestone.days<=30).length},items});
    }
    if (path === "launch-readiness" && method === "GET" && isInternal) {
      const investments=await db.sql`SELECT id,inversor,email,portal_user_id,fecha_inicio,fecha_fin,plazo_meses,estado FROM inversiones WHERE estado NOT IN ('Cancelada','Cerrada')`;
      for(const i of investments)await ensureInvestorChecklist(i.id);
      const [docStats]=await db.sql`SELECT COUNT(*)::int AS total,COUNT(*) FILTER(WHERE obligatorio=TRUE AND estado='Validado')::int AS valid,COUNT(*) FILTER(WHERE obligatorio=TRUE AND estado<>'Validado')::int AS pending,COUNT(*) FILTER(WHERE estado='Pendiente de revisión')::int AS review,COUNT(*) FILTER(WHERE estado='Caducado')::int AS expired FROM inversion_document_requirements q JOIN inversiones i ON i.id=q.inversion_id WHERE i.estado NOT IN ('Cancelada','Cerrada')`;
      const [appt]=await db.sql`SELECT COUNT(*) FILTER(WHERE status='Solicitada')::int AS requested,COUNT(*) FILTER(WHERE status='Solicitada' AND created_at<NOW()-INTERVAL '24 hours')::int AS overdue FROM investor_appointments`;
      const [tickets]=await db.sql`SELECT COUNT(*) FILTER(WHERE estado='Abierto')::int AS open,COUNT(*) FILTER(WHERE estado='Abierto' AND created_at<NOW()-INTERVAL '24 hours')::int AS overdue FROM tickets`;
      const [mail]=await db.sql`SELECT COUNT(*) FILTER(WHERE active=TRUE AND encrypted_refresh_token IS NOT NULL)::int AS connected,COUNT(*) FILTER(WHERE active=TRUE AND last_error IS NOT NULL)::int AS errors FROM mail_accounts`;
      const decisions=await db.sql`SELECT inversion_id,hito FROM inversion_decisiones`,decisionKeys=new Set(decisions.map(d=>d.inversion_id+":"+(d.hito||"vencimiento_final")));let due=0;for(const i of investments)for(const h of investmentMilestones(invRow(i))){const days=daysUntil(h.fecha);if(days!=null&&days>=0&&days<=30&&!decisionKeys.has(i.id+":"+h.hito))due++;}
      const missingAccess=investments.filter(i=>!i.email||!i.portal_user_id).length,checks=[
        {key:"access",label:"Acceso de inversores",ok:missingAccess===0,count:missingAccess,detail:missingAccess?missingAccess+" contratos sin acceso completo":"Todos los contratos activos tienen acceso"},
        {key:"documents",label:"Expedientes obligatorios",ok:(docStats.pending||0)===0,count:docStats.pending||0,detail:(docStats.valid||0)+" validados · "+(docStats.review||0)+" por revisar · "+(docStats.expired||0)+" caducados"},
        {key:"decisions",label:"Decisiones próximas",ok:due===0,count:due,detail:due?due+" hitos dentro de 30 días sin decisión":"Sin decisiones contractuales pendientes"},
        {key:"appointments",label:"Citas de inversores",ok:(appt.overdue||0)===0,count:appt.overdue||0,detail:(appt.requested||0)+" solicitadas · "+(appt.overdue||0)+" con más de 24 horas"},
        {key:"support",label:"Solicitudes de soporte",ok:(tickets.overdue||0)===0,count:tickets.overdue||0,detail:(tickets.open||0)+" abiertas · "+(tickets.overdue||0)+" con más de 24 horas"},
        {key:"mail",label:"Correo corporativo",ok:(mail.connected||0)>0&&(mail.errors||0)===0,count:mail.errors||0,detail:(mail.connected||0)+" buzones conectados · "+(mail.errors||0)+" con error"},
      ];const passed=checks.filter(c=>c.ok).length,score=Math.round(passed/checks.length*100),incidents=await db.sql`SELECT i.*,u.name AS assigned_name FROM launch_incidents i LEFT JOIN usuarios u ON u.id=i.assigned_user_id ORDER BY CASE i.status WHEN 'Abierta' THEN 0 WHEN 'En curso' THEN 1 ELSE 2 END,CASE i.severity WHEN 'critica' THEN 0 WHEN 'alta' THEN 1 WHEN 'media' THEN 2 ELSE 3 END,i.created_at DESC LIMIT 200`;
      return json({score,status:score===100?"Listo":(score>=75?"Revisión necesaria":"No listo"),checks,incidents:incidents.map(i=>({id:i.id,area:i.area,severity:i.severity,title:i.title,detail:i.detail,entityType:i.entity_type,entityId:i.entity_id,status:i.status,assignedName:i.assigned_name,createdAt:i.created_at,updatedAt:i.updated_at}))});
    }
    if (path === "launch-incidents" && method === "POST" && isInternal) {
      const b=await req.json().catch(()=>({})),title=String(b.title||"").trim().slice(0,200);if(!title)return json({error:"Indica un título"},400);const severity=["baja","media","alta","critica"].includes(b.severity)?b.severity:"media",id=uid("issue");await db.sql`INSERT INTO launch_incidents(id,area,severity,title,detail,entity_type,entity_id,reporter_id,assigned_user_id) VALUES(${id},${String(b.area||"Inversores").slice(0,80)},${severity},${title},${String(b.detail||"").slice(0,3000)},${String(b.entityType||"").slice(0,80)||null},${String(b.entityId||"").slice(0,160)||null},${user.id},${b.assignedUserId?parseInt(b.assignedUserId,10):null})`;return json({ok:true,id});
    }
    if (seg[0] === "launch-incidents" && seg[1] && method === "POST" && isInternal) {
      const b=await req.json().catch(()=>({})),status=["Abierta","En curso","Resuelta"].includes(b.status)?b.status:"";if(!status)return json({error:"Estado no válido"},400);await db.sql`UPDATE launch_incidents SET status=${status},assigned_user_id=${b.assignedUserId?parseInt(b.assignedUserId,10):null},resolved_at=${status==="Resuelta"?new Date().toISOString():null},updated_at=NOW() WHERE id=${seg[1]}`;return json({ok:true});
    }
    if (path === "mi-informe-patrimonial" && method === "GET") {
      ...
      const contracts=rows.map(invRow),capital=contracts.reduce((s,c)=>s+aed(c.capital,c.moneda),0),paid=contracts.reduce((s,c)=>s+(c.pagos||[]).reduce((x,p)=>x+aed(p.importe,p.moneda||c.moneda),0),0),returned=contracts.reduce((s,c)=>s+(c.devoluciones||[]).reduce((x,p)=>x+aed((Number(p.capital)||0)+(Number(p.intereses)||0)+(Number(p.importe)||0),p.moneda||c.moneda),0),0),projected=contracts.reduce((s,c)=>s+aed(c.capital,c.moneda)*(Number(c.rentabilidad)||0)/100,0);
      const milestones=contracts.flatMap(c=>investmentMilestones(c).map(h=>({project:c.proyecto||"Inversión",title:h.titulo,date:h.fecha,returnPct:h.retorno}))).filter(h=>daysUntil(h.date)>=0).sort((a,b)=>String(a.date).localeCompare(String(b.date))).slice(0,8);
      const table=contracts.length?contracts.map(c=>'<tr><td><b>'+safeText(c.proyecto||"Inversión")+'</b>'+(c.unidad?'<small>'+safeText(c.unidad)+'</small>':'')+'</td><td>'+safeText(c.estado||"Activa")+'</td><td>'+safeText(c.fechaInicio||"—")+'</td><td>'+safeText(c.fechaFin||"—")+'</td><td class="num">'+safeText(c.capital)+' '+safeText(c.moneda||"AED")+'</td><td class="num">'+safeText(c.rentabilidad||0)+'%</td></tr>').join(''):'<tr><td colspan="6">No hay contratos vinculados.</td></tr>';
      const timeline=milestones.length?milestones.map(h=>'<li><span>'+safeText(h.date)+'</span><b>'+safeText(h.title)+' · '+safeText(h.project)+'</b><em>Retorno aplicable '+safeText(h.returnPct)+'%</em></li>').join(''):'<li><b>No existen hitos futuros registrados.</b></li>';
